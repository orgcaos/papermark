...
// Pretty short-URL sibling of pages/view/[linkId]/index.tsx -- same viewer
// page, keyed by the link's shortSlug (e.g. /l/services-book-a8k2) instead
// of its cuid. Kept as a parallel file (matching how
// pages/view/domains/[domain]/[slug] already parallels pages/view/[linkId])
// rather than a redirect, so the pretty URL stays in the address bar.
...
export const getStaticProps = async (context: GetStaticPropsContext) => {
  const { shortSlug: shortSlugParam } = context.params as {
    shortSlug: string;
  };

  try {
    // shortSlugs are generated as {slugified-name}-{4-char-suffix} (see
    // lib/utils/generate-short-slug.ts) -- lowercase letters/digits and
    // hyphens only, never a cuid.
    const shortSlug = z
      .string()
      .min(1)
      .max(80)
    ...
    const result = await fetchLinkDataByShortSlug({ shortSlug });
    ...
    const { linkType, link, brand, publicMeta, linkId } = result;
    ...
    if (linkType === "WORKFLOW_LINK") {
      return {
        props: {
          linkData: {
            linkType: "WORKFLOW_LINK",
            // The real link cuid -- WorkflowAccessView calls
            // /api/workflow-entry/link/{entryLinkId}/*, which needs the
            // actual id, not the shortSlug used in the URL.
            entryLinkId: linkId!,
            brand: brand || null,
          },
          notionData: {
    ...
    if (linkType === "DOCUMENT_LINK") {
      ...
      return {
        props: {
          ...
          // Always off for this single-tenant deployment - a client
          // opening a shared link should never see "Powered by" branding
          // or an account-creation upsell slide.
          showPoweredByBanner: false,
          showAccountCreationSlide: false,
...
function ViewPageInner({
  ...
  useEffect(() => {
    ...
    const cookieToken =
      Cookies.get("pm_vft") ||
      Cookies.get(`pm_drs_flag_${router.query.shortSlug}`);
    ...
    return (
      <>
        <CustomMetaTag
          ...
          title="Access Workflow | Orgcaos Docket"
          description={null}
          imageUrl={null}
          url={meta.metaUrl ?? ""}
        />
      ...
      return (
        <>
          <CustomMetaTag
            ...
            title={
              meta.metaTitle ?? `${link?.document?.name} | Orgcaos Docket`
            }
      ...
      return (
        <>
          <CustomMetaTag
            ...
            enableBranding={meta.enableCustomMetatag ?? false}
            title={
              meta.metaTitle ?? `${link?.dataroom?.name} | Orgcaos Docket`
            }
            description={meta.metaDescription ?? null}
            imageUrl={meta.metaImage ?? null}
            url={meta.metaUrl ?? ""}
